...
Überschriften bekommen ihre Display-Laufweite (`-0.025em`) direkt vom Theme. Egal, was du für `display` wählst — auch Textschriften wie der Inter-Standard — Überschriften wirken in großen Größen richtig gesetzt, ohne dass die Laufweite in der Schrift selbst stecken muss.
