...
| `provider` | `model` | API कुंजी env चर |
| --- | --- | --- |
| `gateway` (डिफ़ॉल्ट) | AI Gateway के माध्यम से एक `provider/model` स्ट्रिंग | `AI_GATEWAY_API_KEY` |
| `openrouter` | कोई भी [OpenRouter](https://openrouter.ai) मॉडल | `OPENROUTER_API_KEY` |
| `llmgateway` | कोई भी [LLMGateway](https://llmgateway.io) मॉडल | `LLMGATEWAY_API_KEY` |
| `inkeep` | एक [Inkeep](https://inkeep.com) QA मॉडल | `INKEEP_API_KEY` |
| `openai-compatible` | जो कुछ भी आपका एंडपॉइंट सर्व करता है | `apiKeyEnv` से सेट करें |
